...
import { getFirstFromMaybeArray, getPrimaryArtistFromRelation } from '@/lib/relations'
...
type ArtistNameOnly = { name: string | null }
...
type VideoLite = {
  ...
  artists_v2: ArtistNameOnly | ArtistNameOnly[] | null
}
...
export default async function EditProfilePage({
  ...
  const initialFavoriteVideos: Tag[] = favoriteVideoRows
    ...
    .map((video) => {
      const artistRel = getFirstFromMaybeArray(video.artists_v2)
      const tag: Tag = {
        type: 'video',
        id: video.id,
        name: video.title ?? '',
