...
// - まず market=JP で検索 → 非日本語候補は ISRC で「グローバル再検索」し、日本語版/JP再生可版を優先
// - それでも英語名しか取れない場合は、“同一ISRCの日本語表記” を JP 検索で取り直す（relabel）
//   *NEW*: relabel 時、track: は「候補トラックの曲名」と「ユーザー入力」の両方を試す
//   *NEW*: アーティストは英名／日本名の両方（環境辞書で相互変換）を試す
//   *NEW*: 最後の保険で「単一 ISRC の JP 検索」を追加
// - 表示名は日本語エイリアス（環境変数）で上書き可能（任意）
...
/** Spotifyっぽいランク：完全一致/前方一致/部分一致＋人気度＋日本語加点 */
...
async function sFetch(
  ...
  dlog(tag, '→', { url: url.toString(), hadMarket, forceNoMarket: !!opts?.forceNoMarket })
...
// 英名⇄日本名の両方を候補にする
function artistNameVariants(input?: string): string[] {
  const set = new Set<string>()
  if (input && input.trim()) set.add(input.trim())
  // 英名→日本名（環境変数の別名辞書）
  if (input && ARTIST_ALIASES.byName[input]) set.add(ARTIST_ALIASES.byName[input])
  // 日本名→英名（byName の逆引き）
  if (input) {
    for (const [en, ja] of Object.entries(ARTIST_ALIASES.byName)) {
      if (ja === input) set.add(en)
    }
  }
...
// trackQueries: 候補トラック名とユーザー入力の両方（重複除去）を試す
async function tryRelabelFromQueryJP(
  ...
  const tq = Array.from(new Set(trackQueries.filter(q => !!q && q.trim().length > 0)))
  const names = Array.from(new Set(artistNames.filter(n => !!n && n.trim().length > 0)))
  // アーティスト指定なしも最後に試す
  names.push('')
  ...
  for (const q of tq) {
    for (const name of names) {
      ...
      if (name) parts.push(`artist:"${name}"`)
      const url = `${SEARCH_ENDPOINT}?q=${encodeURIComponent(parts.join(' '))}&type=track&limit=25`
      ...
      const hit = items.find(t => t.external_ids?.isrc === targetIsrc && hasJPInTrack(t))
      if (hit) return hit
    }
  }

  // 最後の保険：単一 ISRC の JP 検索
  const url2 = `${SEARCH_ENDPOINT}?q=${encodeURIComponent(`isrc:${targetIsrc}`)}&type=track&limit=50`
  const res2 = await sFetch(url2, token, { tag: 'relabelJP:isrc' })
  const data2: SearchTracksResponse = await res2.json()
  let items2: SpotifyTrackRaw[] = data2?.tracks?.items ?? []
  items2 = await fillTracksDetails(items2, token)
  const hit2 = items2.find(t => t.external_ids?.isrc === targetIsrc && hasJPInTrack(t))
  return hit2 ?? null
}
...
export const searchTracks = async (query: string): Promise<SearchTrack[]> => {
  ...
  const isrcs = Array.from(new Set(
    needFallback
  ...
  // trackQueries: 候補曲名（t.name）＋ ユーザー入力（query）の両方を試す
  let relabeled = 0
  for (let i = 0; i < top.length && relabeled < 3; i++) {
    ...
    const artist0 = t.artists?.[0]?.name
    const variants = artistNameVariants(artist0) // 英名/日本名
    const trackQueries = Array.from(new Set([t.name, hasJapanese(query) ? query : ''].filter(s => !!s && s.trim().length > 0)))
    const jp = await tryRelabelFromQueryJP(trackQueries, variants, access_token, isrc)
    if (jp) { top[i] = jp; relabeled++ }
  }
